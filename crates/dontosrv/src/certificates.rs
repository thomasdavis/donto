...
pub struct AttachResp { pub statement_id: uuid::Uuid, pub kind: String }
...
pub async fn attach(
    ...
    let conn = match pool.get().await {
        Ok(c) => c, Err(e) => return Json(json!({"error": e.to_string()})).into_response(),
    };
    ...
    Json(AttachResp { statement_id: req.statement_id, kind: req.kind }).into_response()
}

#[derive(Debug, Serialize)]
pub struct VerifyResp { pub statement_id: uuid::Uuid, pub kind: String, pub ok: bool, pub reason: Option<String> }
...
pub async fn verify(
    ...
    Json(VerifyResp { statement_id: stmt, kind, ok, reason }).into_response()
}
...
async fn run_verifier(
    ...
    match kind {
        "direct_assertion" => {
            let has_source = body.get("source").and_then(|v| v.as_str()).map_or(false, |s| !s.is_empty());
            if has_source { (true, None) } else { (false, Some("direct_assertion needs body.source".into())) }
        }
        "substitution" => {
            let needed = body.get("substitutes").and_then(|v| v.as_array())
                .map(|arr| arr.iter().filter_map(|x| x.as_str().and_then(|s| uuid::Uuid::parse_str(s).ok())).collect::<Vec<_>>())
                .unwrap_or_default();
            let missing: Vec<_> = needed.iter().filter(|n| !inputs.contains(n)).collect();
            if missing.is_empty() { (true, None) } else { (false, Some(format!("missing inputs: {missing:?}"))) }
        }
        "transitive_closure" => {
        ...
        "shape_entailment" => {
            ...
            if cached.is_some() { (true, None) } else { (false, Some(format!("no shape report for {shape_iri}"))) }
        }
        "confidence_justification" => {
            // body: {tier: "moderate", source_count: 3}
            let _ = (rule_iri, body); (true, None)
        }
        "hypothesis_scoped" => {
            let hctx = body.get("hypothesis").and_then(|v| v.as_str()).unwrap_or("");
            if hctx.is_empty() { (false, Some("body.hypothesis required".into())) } else { (true, None) }
        }
        "replay" => {
            // Re-run rule_iri with body.scope into body.into; succeed if any
            // emitted statement has the same content as `stmt`.
            (rule_iri.is_some(), rule_iri.is_none().then(|| "rule_iri required".into()))
        }
...
async fn verify_transitive(
    ...
    let conn = match state.client.pool().get().await {
        Ok(c) => c, Err(e) => return (false, Some(e.to_string())),
    };
    let row = match conn.query_opt(
        "select subject, object_iri from donto_statement where statement_id = $1",
        &[&stmt],
    ).await { Ok(Some(r)) => r, _ => return (false, Some("statement not found".into())) };
    let subj: String = row.get(0);
    let obj:  Option<String> = row.get(1);
    let Some(obj) = obj else { return (false, Some("derived statement has no IRI object".into())); };
    ...
    if found.is_some() { (true, None) }
    else { (false, Some(format!("no closure path {subj} -[{predicate}+]-> {obj}"))) }
}
