...
async fn direct_assertion_requires_source() {
    ...
    let id = state
        .client
        .assert(&StatementInput::new("ex:s", "ex:p", Object::iri("ex:o")).with_context(&ctx))
        .await
        .unwrap();
    ...
    assert_eq!(
        v["ok"],
        json!(false),
        "empty direct_assertion must reject: {v}"
    );
}
...
async fn direct_assertion_with_source_passes() {
    ...
    assert_eq!(
        v["ok"],
        json!(true),
        "direct_assertion with source must pass: {v}"
    );
}
...
async fn substitution_requires_inputs_to_match() {
    ...
    let id_target = state
        .client
        .assert(&StatementInput::new("ex:t", "ex:derived", Object::iri("ex:y")).with_context(&ctx))
        .await
        .unwrap();
    let id_input = state
        .client
        .assert(&StatementInput::new("ex:i", "ex:input", Object::iri("ex:x")).with_context(&ctx))
        .await
        .unwrap();
    let app = dontosrv::router(state.clone());
...
async fn transitive_closure_certificate_walks_real_path() {
    ...
    c.assert(&StatementInput::new("ex:b", "ex:parent", Object::iri("ex:c")).with_context(&ctx))
        ...
        .unwrap();
    let id_derived = c
        .assert(&StatementInput::new("ex:a", "ex:parent+", Object::iri("ex:c")).with_context(&ctx))
        .await
        .unwrap();
