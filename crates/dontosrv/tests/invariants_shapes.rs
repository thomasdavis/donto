...
async fn validation_never_deletes_or_modifies_statements() {
    ...
    let c = &state.client;
    let id1 = c.assert(&StatementInput::new("ex:a","ex:spouse",Object::iri("ex:b")).with_context(&ctx)).await.unwrap();
    let id2 = c.assert(&StatementInput::new("ex:a","ex:spouse",Object::iri("ex:c")).with_context(&ctx)).await.unwrap();
...
async fn datatype_shape_finds_mismatched_literals() {
    ...
    c.assert(&StatementInput::new("ex:a","ex:age",
        Object::lit(donto_client::Literal::integer(36))).with_context(&ctx)).await.unwrap();
    c.assert(&StatementInput::new("ex:b","ex:age",
        Object::lit(donto_client::Literal::string("forty"))).with_context(&ctx)).await.unwrap();
    c.assert(&StatementInput::new("ex:c","ex:age",
        Object::iri("ex:hidden_iri")).with_context(&ctx)).await.unwrap();
