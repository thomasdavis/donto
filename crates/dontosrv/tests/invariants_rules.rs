...
async fn derive(state: Arc<dontosrv::AppState>, rule: &str, ctx: &str, into: &str) -> Value {
    ...
    let req = Request::builder().method("POST").uri("/rules/derive")
...
async fn transitive_closure_derives_full_set() {
    ...
    for (s, o) in [("ex:a","ex:b"), ("ex:b","ex:c"), ("ex:c","ex:d")] {
        c.assert(&StatementInput::new(s,"ex:parent",Object::iri(o)).with_context(&ctx)).await.unwrap();
    }
...
async fn rerun_with_identical_inputs_returns_cached() {
    let Some((state, ctx)) = boot().await else { eprintln!("skip"); return; };
    let c = &state.client;
    c.assert(&StatementInput::new("ex:a","ex:p",Object::iri("ex:b")).with_context(&ctx)).await.unwrap();
    let into = format!("ctx:der:{}", uuid::Uuid::new_v4().simple());
    ...
    assert_eq!(v2.get("source").and_then(|x| x.as_str()), Some("cached"),
...
async fn lineage_pointers_recorded_for_derived_statements() {
    let Some((state, ctx)) = boot().await else { eprintln!("skip"); return; };
    let c = &state.client;
    let id_ab = c.assert(&StatementInput::new("ex:a","ex:p",Object::iri("ex:b")).with_context(&ctx)).await.unwrap();
    let id_bc = c.assert(&StatementInput::new("ex:b","ex:p",Object::iri("ex:c")).with_context(&ctx)).await.unwrap();
    let into = format!("ctx:der:{}", uuid::Uuid::new_v4().simple());
    ...
    assert!(lineage.contains(&id_ab) && lineage.contains(&id_bc),
...
async fn derived_statement_carries_higher_maturity() {
    ...
    assert_eq!(mat, 3, "derived statements must carry maturity = 3 (Level 3 of the ladder)");
}
...
async fn unknown_rule_iri_errors_without_writing() {
    ...
    assert_eq!(n, 0, "errored derive must not write into the output context");
}
