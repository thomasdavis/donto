...
async fn health_endpoint() {
    let Some((state, _ctx)) = bootstrap().await else { eprintln!("skip"); return; };
...
async fn sparql_endpoint_runs_query() {
    let Some((state, ctx)) = bootstrap().await else { eprintln!("skip"); return; };
    let c = &state.client;
    c.assert(&StatementInput::new("ex:s", "ex:p", Object::iri("ex:o")).with_context(&ctx)).await.unwrap();
    ...
    let (status, v) = post_json(app, "/dontoql", json!({
...
async fn shape_validate_functional_finds_violations() {
    let Some((state, ctx)) = bootstrap().await else { eprintln!("skip"); return; };
    let c = &state.client;
    c.assert(&StatementInput::new("ex:alice", "ex:spouse", Object::iri("ex:bob"))
        .with_context(&ctx)).await.unwrap();
    c.assert(&StatementInput::new("ex:alice", "ex:spouse", Object::iri("ex:carol"))
        .with_context(&ctx)).await.unwrap();

    let app = dontosrv::router(state.clone());
    let (status, v) = post_json(app, "/shapes/validate", json!({
    ...
    if let Some(viols) = v.get("violations").and_then(|x| x.as_array()) {
        ...
        assert_eq!(viols.len(), 1);
    } else { panic!("unexpected response: {v}"); }
}
...
async fn rule_derive_transitive_closure_emits_into_context() {
    ...
    c.assert(&StatementInput::new("ex:c", "ex:parent", Object::iri("ex:d")).with_context(&ctx)).await.unwrap();

    let into = format!("ctx:derived:{}", uuid::Uuid::new_v4().simple());
    let app = dontosrv::router(state.clone());
    let (status, v) = post_json(app, "/rules/derive", json!({
...
async fn certificate_attach_and_verify_direct() {
    let Some((state, ctx)) = bootstrap().await else { eprintln!("skip"); return; };
    let c = &state.client;
    let id = c.assert(&StatementInput::new("ex:s", "ex:claim", Object::lit(Literal::string("hi")))
        .with_context(&ctx)).await.unwrap();

    let app = dontosrv::router(state.clone());
    let (st, _v) = post_json(app.clone(), "/certificates/attach", json!({
        "statement_id": id,
