...
async fn emit_and_resolve_lifecycle() {
    ...
    assert!(row.get::<_, Option<chrono::DateTime<chrono::Utc>>>("resolved_at").is_some());
}
...
async fn resolve_already_resolved_is_noop() {
    ...
    client.resolve_obligation(obl_id, None, "resolved").await.unwrap();
    let second = client.resolve_obligation(obl_id, None, "resolved").await.unwrap();
    assert!(!second, "resolving an already-resolved obligation must return false");
}
...
async fn assign_obligation_to_agent() {
    ...
    let agent_id = client.ensure_agent(&agent_iri, "llm", None, None).await.unwrap();
    ...
    let pool = client.pool();
    let c = pool.get().await.unwrap();
    c.execute("select donto_assign_obligation($1, $2)", &[&obl_id, &agent_id])
        .await.unwrap();
    ...
    assert_eq!(row.get::<_, Option<uuid::Uuid>>("assigned_agent"), Some(agent_id));
}
...
async fn open_obligations_filtered() {
    ...
    for t in ["needs-coref", "needs-coref", "needs-temporal-grounding"] {
        client.emit_obligation(stmt_id, t, &ctx, 0, None, None).await.unwrap();
    }
...
async fn obligation_summary() {
    ...
    let o1 = client.emit_obligation(stmt_id, "needs-coref", &ctx, 0, None, None).await.unwrap();
    client.emit_obligation(stmt_id, "needs-coref", &ctx, 0, None, None).await.unwrap();
    client.emit_obligation(stmt_id, "needs-source-support", &ctx, 0, None, None).await.unwrap();

    // Resolve one coref.
    client.resolve_obligation(o1, None, "resolved").await.unwrap();

    let pool = client.pool();
    let c = pool.get().await.unwrap();
    let rows = c
        .query("select obligation_type, status, cnt from donto_obligation_summary($1)", &[&ctx])
        .await.unwrap();

    let find = |typ: &str, status: &str| -> i64 {
        rows.iter()
            .find(|r| r.get::<_, String>("obligation_type") == typ
