...
async fn descendants_are_visible_from_root_scope() {
    ...
    c.ensure_context(&root,  "custom", "permissive", None).await.unwrap();
    c.ensure_context(&child, "custom", "permissive", Some(&root)).await.unwrap();
    c.ensure_context(&grand, "custom", "permissive", Some(&child)).await.unwrap();

    for ctx in [&root, &child, &grand] {
        c.assert(&StatementInput::new("ex:s","ex:p",Object::iri(ctx)).with_context(ctx))
            .await.unwrap();
    }
    let n = c.match_pattern(Some("ex:s"), Some("ex:p"), None,
        Some(&ContextScope::just(&root)), None, 0, None, None).await.unwrap().len();
...
async fn exclude_wins_over_descendant_inclusion() {
    ...
    let rows = c.match_pattern(Some("ex:s"), Some("ex:p"), None,
    ...
    let objs: Vec<String> = rows.into_iter().map(|r| match r.object {
        Object::Iri(s) => s,
        _ => unreachable!(),
    }).collect();
    assert!(objs.contains(&root),  "root must be visible");
    assert!(!objs.contains(&child), "child must be excluded");
    assert!(objs.contains(&grand), "grandchild must be visible (excludes are per-context, not subtree prunes)");
}
...
async fn ancestor_walk_optional_off_by_default() {
    ...
    c.assert(&StatementInput::new("ex:s","ex:p",Object::iri("ex:from_root"))
        .with_context(&root)).await.unwrap();
    c.assert(&StatementInput::new("ex:s","ex:p",Object::iri("ex:from_child"))
        .with_context(&child)).await.unwrap();
    ...
    let with_anc = ContextScope::just(&child).without_descendants().with_ancestors();
    let n = c.match_pattern(Some("ex:s"), Some("ex:p"), None,
        Some(&with_anc), None, 0, None, None).await.unwrap().len();
...
async fn curated_context_rejects_unregistered_predicate() {
    ...
    assert!(r.is_err(), "curated context must reject unregistered predicates");
}
