...
async fn contradictions_coexist() {
    ...
    client.ensure_context(&src_a, "source", "permissive", None).await.unwrap();
    client.ensure_context(&src_b, "source", "permissive", None).await.unwrap();

    client.assert(&StatementInput::new("ex:alice", "ex:birthYear",
        Object::lit(donto_client::Literal::integer(1899)))
        .with_context(&src_a)).await.unwrap();
    client.assert(&StatementInput::new("ex:alice", "ex:birthYear",
        Object::lit(donto_client::Literal::integer(1925)))
        .with_context(&src_b)).await.unwrap();
...
async fn negated_hidden_by_default() {
    ...
    client.assert(&StatementInput::new("ex:alice", "ex:member", Object::iri("ex:club"))
        .with_context(&ctx)
        .with_polarity(Polarity::Negated)).await.unwrap();

    let scope = ContextScope::just(&ctx);

    // Default polarity = asserted → hidden.
    let rows = client.match_pattern(
        Some("ex:alice"), Some("ex:member"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None,
    ).await.unwrap();
    assert!(rows.is_empty(), "negated statement must be hidden by default");

    // Opt-in: see it.
    let rows = client.match_pattern(
        Some("ex:alice"), Some("ex:member"), None,
        Some(&scope), Some(Polarity::Negated), 0, None, None,
    ).await.unwrap();
