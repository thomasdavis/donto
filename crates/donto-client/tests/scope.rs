...
async fn descendants_are_included_by_default() {
    ...
    client.assert(&StatementInput::new("ex:s", "ex:p", Object::iri("ex:child"))
        .with_context(&child)).await.unwrap();
    client.assert(&StatementInput::new("ex:s", "ex:p", Object::iri("ex:grand"))
        .with_context(&grand)).await.unwrap();

    let scope = ContextScope::just(&root);
    let rows = client.match_pattern(
        Some("ex:s"), Some("ex:p"), None,
...
async fn ancestors_opt_in() {
    ...
    let scope = ContextScope::just(&child).without_descendants().with_ancestors();
    let rows = client.match_pattern(
        Some("ex:s"), Some("ex:p"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None,
    ).await.unwrap();
