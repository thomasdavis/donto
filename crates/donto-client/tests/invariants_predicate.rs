...
async fn implicit_registration_in_permissive_context() {
    ...
    c.assert(&StatementInput::new("ex:s", &unique, Object::iri("ex:o"))
        .with_context(&ctx)).await.unwrap();

    let conn = c.pool().get().await.unwrap();
    let row = conn.query_one(
        "select status from donto_predicate where iri = $1", &[&unique.as_str()],
    ).await.unwrap();
...
async fn explicit_registration_promotes_implicit_to_active() {
    ...
    // First use: implicit.
    c.assert(&StatementInput::new("ex:s", &pred, Object::iri("ex:o"))
        .with_context(&ctx)).await.unwrap();
...
async fn alias_chain_rejected() {
    ...
    conn.execute("select donto_register_predicate($1,null,null,null,null,null,null,null)", &[&canon.as_str()]).await.unwrap();
    conn.execute("select donto_register_predicate($1,null,null,$2,null,null,null,null)", &[&alias_a.as_str(), &canon.as_str()]).await.unwrap();
    let r = conn.execute(
...
async fn canonical_lookup_resolves_alias() {
    ...
    conn.execute("select donto_register_predicate($1,null,null,null,null,null,null,null)", &[&canon.as_str()]).await.unwrap();
    conn.execute("select donto_register_predicate($1,null,null,$2,null,null,null,null)", &[&alias.as_str(), &canon.as_str()]).await.unwrap();

    let r1: String = conn.query_one(
        "select donto_canonical_predicate($1)", &[&alias.as_str()],
    ).await.unwrap().get(0);
    assert_eq!(r1, canon, "alias must resolve to canonical");

    let r2: String = conn.query_one(
        "select donto_canonical_predicate($1)", &[&canon.as_str()],
    ).await.unwrap().get(0);
    assert_eq!(r2, canon, "canonical resolves to itself");

    let unknown = format!("ex:never_seen_{}", uuid::Uuid::new_v4().simple());
    let r3: String = conn.query_one(
        "select donto_canonical_predicate($1)", &[&unknown.as_str()],
    ).await.unwrap().get(0);
    assert_eq!(r3, unknown, "unregistered predicate is its own canonical (open-world)");
}
...
async fn match_canonical_unifies_alias_siblings() {
    ...
    conn.execute("select donto_register_predicate($1,null,null,null,null,null,null,null)", &[&canon.as_str()]).await.unwrap();
    conn.execute("select donto_register_predicate($1,null,null,$2,null,null,null,null)", &[&alias_a.as_str(), &canon.as_str()]).await.unwrap();
    conn.execute("select donto_register_predicate($1,null,null,$2,null,null,null,null)", &[&alias_b.as_str(), &canon.as_str()]).await.unwrap();

    // Insert one statement under each form.
    for p in [&canon, &alias_a, &alias_b] {
        c.assert(&StatementInput::new("ex:s", p, Object::iri("ex:o"))
            .with_context(&ctx)).await.unwrap();
    }

    // donto_match (raw) returns only the asked predicate.
    let n = c.match_pattern(Some("ex:s"), Some(&canon), None,
