...
async fn retraction_never_deletes_the_row() {
    ...
    let id = c.assert(&StatementInput::new("ex:a","ex:p",Object::iri("ex:b")).with_context(&ctx))
        .await.unwrap();
...
async fn double_retract_is_idempotent_and_silent() {
    ...
    assert!(!c.retract(id).await.unwrap(), "second retract returns false (no row affected)");
...
async fn as_of_anywhere_in_open_window_returns_row() {
    ...
    for offset_ms in [10i64, 50, 100, 200, 500] {
        ...
        let n = c.match_pattern(Some("ex:a"), Some("ex:p"), None,
            Some(&ContextScope::just(&ctx)), Some(Polarity::Asserted), 0,
            Some(probe), None,
        ).await.unwrap().len();
...
async fn as_of_after_retraction_does_not_show_row() {
    ...
    let pre = c.match_pattern(Some("ex:a"), Some("ex:p"), None,
        Some(&ContextScope::just(&ctx)), Some(Polarity::Asserted), 0,
        Some(before_retract), None).await.unwrap();
    assert_eq!(pre.len(), 1, "pre-retraction window must see the row");

    let post = c.match_pattern(Some("ex:a"), Some("ex:p"), None,
        Some(&ContextScope::just(&ctx)), Some(Polarity::Asserted), 0,
        Some(after_retract), None).await.unwrap();
...
async fn correction_creates_new_row_and_closes_prior() {
    ...
    let id_old = c.assert(&StatementInput::new("ex:a","ex:loc",Object::iri("ex:wrong"))
        .with_context(&ctx)).await.unwrap();
    let id_new = c.correct(id_old, None, None, Some(&Object::iri("ex:right")), None).await.unwrap();
    ...
    let rows = c.match_pattern(Some("ex:a"), Some("ex:loc"), None,
        Some(&ContextScope::just(&ctx)), Some(Polarity::Asserted), 0, None, None).await.unwrap();
...
async fn valid_time_intervals_are_independent_of_tx_time() {
    ...
    let stages = [
        (NaiveDate::from_ymd_opt(1970, 1, 1).unwrap(), NaiveDate::from_ymd_opt(1990, 1, 1).unwrap(), "ex:youth"),
        (NaiveDate::from_ymd_opt(1990, 1, 1).unwrap(), NaiveDate::from_ymd_opt(2010, 1, 1).unwrap(), "ex:middle"),
        (NaiveDate::from_ymd_opt(2010, 1, 1).unwrap(), NaiveDate::from_ymd_opt(2030, 1, 1).unwrap(), "ex:senior"),
    ];
    for (lo, hi, role) in stages {
        c.assert(&StatementInput::new("ex:alice","ex:role",Object::iri(role))
            .with_context(&ctx).with_valid(Some(lo), Some(hi))).await.unwrap();
    }
    ...
    for (probe, expected) in cases {
        let rows = c.match_pattern(Some("ex:alice"), Some("ex:role"), None,
            Some(&ContextScope::just(&ctx)), Some(Polarity::Asserted), 0,
            None, Some(probe)).await.unwrap();
...
async fn audit_records_assert_and_retract_actions() {
    ...
    let conn = c.pool().get().await.unwrap();
    let actions: Vec<String> = conn.query(
        "select action from donto_audit where statement_id = $1 order by at",
        &[&id],
    ).await.unwrap().into_iter().map(|r| r.get::<_, String>(0)).collect();
    assert_eq!(actions, vec!["assert", "retract"],
