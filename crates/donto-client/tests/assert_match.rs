...
async fn assert_then_match() {
    ...
    let rows = client.match_pattern(
        Some("ex:alice"), Some("ex:knows"), None,
...
async fn literal_object_round_trips() {
    ...
    let s = StatementInput::new(
        "ex:alice", "ex:age", Object::lit(Literal::integer(36)),
    ).with_context(&ctx);
    client.assert(&s).await.unwrap();

    let scope = ContextScope::just(&ctx);
    let rows = client.match_pattern(
        Some("ex:alice"), Some("ex:age"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None,
    ).await.unwrap();
    assert_eq!(rows.len(), 1);
    match &rows[0].object {
...
async fn batch_assert_counts() {
    ...
    let inputs: Vec<StatementInput> = (0..50).map(|i| {
        StatementInput::new(
            format!("ex:s/{i}"),
            "ex:p",
