...
async fn register_and_resolve_alias() {
    ...
    let resolved: String = c.query_one(
        "select donto_resolve_entity($1)", &[&alias],
    ).await.unwrap().get(0);
...
async fn unregistered_resolves_to_self() {
    ...
    let resolved: String = c.query_one(
        "select donto_resolve_entity($1)", &[&iri],
    ).await.unwrap().get(0);
...
async fn no_self_alias() {
    ...
    let err = c.execute(
        "select donto_register_entity_alias($1, $1)", &[&iri],
    ).await.err().expect("self-alias must error");
...
async fn idempotent_registration() {
    ...
    c.execute("select donto_register_entity_alias($1, $2, 'system-a', 0.8)",
        &[&alias, &canonical]).await.unwrap();
    c.execute("select donto_register_entity_alias($1, $2, 'system-a', 0.95)",
        &[&alias, &canonical]).await.unwrap();
...
async fn query_aliases_both_directions() {
    ...
    c.execute("select donto_register_entity_alias($1, $2, 'sys1')", &[&alias1, &canonical]).await.unwrap();
    c.execute("select donto_register_entity_alias($1, $2, 'sys2')", &[&alias2, &canonical]).await.unwrap();

    // Query from canonical → finds both aliases
    let rows = c.query("select * from donto_entity_aliases($1)", &[&canonical]).await.unwrap();
    assert_eq!(rows.len(), 2);

    // Query from alias1 → finds the pair
    let rows = c.query("select * from donto_entity_aliases($1)", &[&alias1]).await.unwrap();
    assert!(rows.len() >= 1);
}
...
async fn highest_confidence_wins_resolution() {
    ...
    c.execute("select donto_register_entity_alias($1, $2, 'sys', 0.7)", &[&alias, &c1]).await.unwrap();
    c.execute("select donto_register_entity_alias($1, $2, 'sys', 0.95)", &[&alias, &c2]).await.unwrap();

    let resolved: String = c.query_one(
        "select donto_resolve_entity($1)", &[&alias],
    ).await.unwrap().get(0);
    assert_eq!(resolved, c2, "should resolve to highest confidence canonical");
}
