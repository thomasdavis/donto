...
fn fixture_seed_path() -> PathBuf {
    ...
    p.pop(); p.pop();   // crates/donto-migrate/ → repo root
...
async fn migrator_round_trips_every_prd_section_24_rule() {
    let Some((c, root, dbpath)) = boot().await else { eprintln!("skip"); return; };
    let dsn = dsn();

    let out = run_migrator(&dsn, &dbpath, &root);
    assert!(out.status.success(),
        "migrator failed: stderr={}", String::from_utf8_lossy(&out.stderr));
    let report: serde_json::Value = serde_json::from_slice(&out.stdout)
        .unwrap_or_else(|_| panic!("parse migrator stdout: {}", String::from_utf8_lossy(&out.stdout)));

    // 1. Two source contexts created.
    assert_eq!(report["source_contexts"], serde_json::json!(2));
    let conn = c.pool().get().await.unwrap();
    let n_src: i64 = conn.query_one(
    ...
    let labels = c.match_pattern(None, Some("rdfs:label"), None, Some(&scope),
        Some(Polarity::Asserted), 0, None, None).await.unwrap();
    let label_objs: Vec<String> = labels.iter().filter_map(|s| match &s.object {
    ...
    let strong = c.match_pattern(
        Some(&format!("{root}/iri/alice_old")), Some("ex:birthYear"), None,
        Some(&scope), Some(Polarity::Asserted), 1, None, None,
    ).await.unwrap();
    assert_eq!(strong.len(), 1, "strong-confidence claim must reach maturity 1");
    assert_eq!(strong[0].context, format!("{root}/source/src/census1900"));

    let speculative = c.match_pattern(
        Some(&format!("{root}/iri/alice_young")), Some("ex:birthYear"), None,
        Some(&scope), Some(Polarity::Asserted), 1, None, None,
    ).await.unwrap();
    assert_eq!(speculative.len(), 0, "speculative claim must NOT pass maturity ≥ 1 floor");
    let any = c.match_pattern(
        Some(&format!("{root}/iri/alice_young")), Some("ex:birthYear"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None,
    ).await.unwrap();
    assert_eq!(any.len(), 1, "speculative claim still present at maturity 0");
    ...
    let typ = c.match_pattern(Some(&marriage), Some("rdf:type"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None).await.unwrap();
    assert!(typ.iter().any(|s| s.object == Object::iri("ex:Marriage")));
    let spouses = c.match_pattern(Some(&marriage), Some("ex:role/spouse"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None).await.unwrap();
    assert_eq!(spouses.len(), 2, "marriage must have two spouse participants");

    // 6. Relationships → donto:sameAs/possiblySame/differentFrom.
    assert_eq!(report["relationships"], serde_json::json!(3));
    let sa = c.match_pattern(None, Some("donto:sameAs"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None).await.unwrap();
    assert_eq!(sa.len(), 1);
    let df = c.match_pattern(None, Some("donto:differentFrom"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None).await.unwrap();
    assert_eq!(df.len(), 1);
    let ps = c.match_pattern(None, Some("donto:possiblySame"), None,
        Some(&scope), Some(Polarity::Asserted), 0, None, None).await.unwrap();
    assert_eq!(ps.len(), 1);

    // 7. Aliases come back with bitemporal valid_time bounded by year_start/end.
    assert_eq!(report["aliases"], serde_json::json!(2));
    let allie = c.match_pattern(
        Some(&format!("{root}/iri/alice_young")), Some("ex:knownAs"), None,
        Some(&scope), Some(Polarity::Asserted), 0,
        None, Some(chrono::NaiveDate::from_ymd_opt(1910, 6, 1).unwrap()),
    ).await.unwrap();
    assert_eq!(allie.len(), 1, "alias must be visible during its valid_time");
    let allie_out = c.match_pattern(
        Some(&format!("{root}/iri/alice_young")), Some("ex:knownAs"), None,
        Some(&scope), Some(Polarity::Asserted), 0,
        None, Some(chrono::NaiveDate::from_ymd_opt(1925, 1, 1).unwrap()),
    ).await.unwrap();
    assert_eq!(allie_out.len(), 0, "alias must NOT match outside its valid_time");

    // 8. Discrepancies → donto:Discrepancy nodes.
    assert_eq!(report["discrepancies"], serde_json::json!(2));
    let discs = c.match_pattern(None, Some("rdf:type"),
        Some("donto:Discrepancy"), Some(&scope), Some(Polarity::Asserted), 0, None, None,
    ).await.unwrap();
    ...
    assert!(emitted > 15, "expected a non-trivial number of statements, got {emitted}");
...
async fn migrator_is_idempotent_on_rerun() {
    ...
    let scope = scope_under(&root);
    let n1 = c.match_pattern(None, None, None, Some(&scope),
        Some(Polarity::Asserted), 0, None, None).await.unwrap().len();

    // Re-run on the same source — every assert should be idempotent.
    run_migrator(&dsn, &dbpath, &root);
    let n2 = c.match_pattern(None, None, None, Some(&scope),
        Some(Polarity::Asserted), 0, None, None).await.unwrap().len();

    assert_eq!(n1, n2, "second migration must not produce new statements (idempotency)");
    let _ = std::fs::remove_file(&dbpath);
}
...
async fn migrator_dry_run_writes_nothing() {
    ...
    let n = c.match_pattern(None, None, None, Some(&scope_under(&root)),
        Some(Polarity::Asserted), 0, None, None).await.unwrap().len();
