...
pub async fn migrate(client: &DontoClient, sqlite: &Path, root: &str, dry_run: bool) -> Result<MigrationReport> {
    ...
    // Root context.
    if !dry_run { client.ensure_context(root, "custom", "permissive", None).await?; }
    ...
    if has_table(&conn, "sources")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            if !dry_run {
                client.ensure_context(&ctx_iri, "source", "permissive", Some(root)).await?;
            }
            if let Some(label) = name {
                all.push(StatementInput::new(&ctx_iri, "rdfs:label", Object::lit(Literal::string(label)))
                    .with_context(root));
            }
            if let Some(c) = citation {
                all.push(StatementInput::new(&ctx_iri, "donto:citation", Object::lit(Literal::string(c)))
                    .with_context(root));
            }
    ...
    if has_table(&conn, "entities")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            let src: Option<String> = r.get::<_, rusqlite::types::Value>(3).ok().map(stringify_value);
            let iri = format!("{root}/iri/{id}");
            let ctx = src.as_deref().map(|s| format!("{root}/source/{s}")).unwrap_or_else(|| root.into());
            if let Some(k) = kind {
                all.push(StatementInput::new(&iri, "rdf:type",   Object::iri(format!("ex:{k}"))).with_context(&ctx));
            }
            if let Some(l) = label {
                all.push(StatementInput::new(&iri, "rdfs:label", Object::lit(Literal::string(l))).with_context(&ctx));
            }
            report.entities += 1;
        }
    }
    ...
    if has_table(&conn, "claims")? {
        let mut stmt = conn.prepare("select subject_id, predicate, object, source_id, confidence from claims")?;
        let mut rows = stmt.query([])?;
        while let Some(r) = rows.next()? {
            ...
            let object = match obj_raw {
                rusqlite::types::Value::Text(s) if s.starts_with("urn:") || s.starts_with("ex:") || s.starts_with("http") =>
                    Object::iri(s),
                rusqlite::types::Value::Integer(i) => Object::lit(Literal::integer(i)),
                rusqlite::types::Value::Real(f)    => Object::lit(Literal { v: serde_json::json!(f), dt: "xsd:decimal".into(), lang: None }),
            ...
            all.push(StatementInput::new(subject, predicate, object).with_context(ctx).with_maturity(maturity));
            report.claims += 1;
        }
    }
    ...
    if has_table(&conn, "events")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            let src: Option<String> = r.get::<_, rusqlite::types::Value>(4).ok().map(stringify_value);
            let ev = format!("{root}/event/{id}");
            let ctx = src.as_deref().map(|s| format!("{root}/source/{s}")).unwrap_or_else(|| root.into());
            if let Some(k) = kind {
                all.push(StatementInput::new(&ev, "rdf:type", Object::iri(format!("ex:{k}"))).with_context(&ctx));
            }
            if let Some(d) = date {
                if let Ok(nd) = parse_date(&d) {
                    all.push(StatementInput::new(&ev, "ex:when", Object::lit(Literal::date(nd))).with_context(&ctx));
                } else {
                    all.push(StatementInput::new(&ev, "ex:whenText", Object::lit(Literal::string(d))).with_context(&ctx));
                }
            }
            if let Some(pl) = place { all.push(StatementInput::new(&ev, "ex:place", Object::lit(Literal::string(pl))).with_context(&ctx)); }
            report.events += 1;
        }
    }
    ...
    if has_table(&conn, "relationships")? {
        let mut stmt = conn.prepare("select left_id, right_id, kind, confidence from relationships")?;
        let mut rows = stmt.query([])?;
        while let Some(r) = rows.next()? {
            ...
            let maturity = if matches!(conf.as_deref(), Some("strong")|Some("moderate")) { 1u8 } else { 0u8 };
            all.push(StatementInput::new(format!("{root}/iri/{left}"), p, Object::iri(format!("{root}/iri/{right}")))
                .with_context(root).with_maturity(maturity));
    ...
    if has_table(&conn, "aliases")? {
        let mut stmt = conn.prepare("select entity_id, name, year_start, year_end, location from aliases")?;
        let mut rows = stmt.query([])?;
        while let Some(r) = rows.next()? {
            let id: String  = r.get::<_, rusqlite::types::Value>(0).map(stringify_value)?;
            ...
            let stmt = StatementInput::new(&ent, "ex:knownAs", Object::lit(Literal::string(&name)))
                .with_context(root).with_valid(lo, hi);
            all.push(stmt);
            if let Some(l) = loc {
                all.push(StatementInput::new(&ent, "ex:knownAsLocation", Object::lit(Literal::string(l)))
                    .with_context(root).with_valid(lo, hi));
            }
            report.aliases += 1;
        }
    ...
    if has_table(&conn, "discrepancies")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            all.push(StatementInput::new(&d, "rdf:type", Object::iri("donto:Discrepancy")).with_context(root));
            if let Some(s) = summary { all.push(StatementInput::new(&d, "rdfs:comment", Object::lit(Literal::string(s))).with_context(root)); }
            if let Some(k) = kind    { all.push(StatementInput::new(&d, "ex:discrepancyKind", Object::lit(Literal::string(k))).with_context(root)); }
    ...
    if has_table(&conn, "hypotheses")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            if !dry_run {
                client.ensure_context(&hctx, "hypothesis", "permissive", Some(root)).await?;
            }
            all.push(StatementInput::new(&h, "rdf:type", Object::iri("donto:Hypothesis")).with_context(root));
            if let Some(s) = statement { all.push(StatementInput::new(&h, "donto:statement", Object::lit(Literal::string(s))).with_context(root)); }
            if let Some(s) = status    { all.push(StatementInput::new(&h, "donto:status",    Object::iri(format!("donto:{s}"))).with_context(root)); }
            report.hypotheses += 1;
        }
    }
