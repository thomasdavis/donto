...
pub fn parse_sparql(src: &str) -> Result<Query, ParseError> {
    ...
    // SELECT clause.
    let kw = tx.bump_word().ok_or_else(|| ParseError("expected SELECT".into()))?;
    ...
    loop {
        tx.skip_ws();
        if tx.peek_char() == Some('*') { tx.bump_char(); break; }
    ...
    // WHERE { ... }.
    let kw = tx.bump_word().ok_or_else(|| ParseError("expected WHERE".into()))?;
    ...
    while let Some(t) = tx.peek_word() {
        ...
        match upper.as_str() {
            "LIMIT"  => { tx.bump_word(); limit  = Some(tx.read_number()?); }
            "OFFSET" => { tx.bump_word(); offset = Some(tx.read_number()?); }
            _ => break,
        }
    }
    ...
    loop {
        ...
        match tx.peek_char() {
            ...
            Some(_) => {
                ...
                if let Some(w) = tx.peek_word() {
                    ...
                    if up == "FILTER" {
                        ...
                        filters.push(match op.as_str() {
                            ...
                            ">"  => Filter::Gt(lhs, rhs),
                            ">=" => Filter::Ge(lhs, rhs),
                            other => return Err(ParseError(format!("unsupported FILTER op `{other}`"))),
                        });
                        continue;
                    }
                    tx.pos = snap;
                ...
                patterns.push(Pattern { subject: s, predicate: p, object: o, graph: current_graph.cloned() });
            }
        }
    }
    Ok((patterns, filters))
...
fn read_term(tx: &mut Tokenizer<'_>, prefixes: &HashMap<String, String>) -> Result<Term, ParseError> {
    tx.skip_ws();
    match tx.peek_char() {
        Some('?') => {
            tx.bump_char();
        ...
        Some('"') => {
            ...
            if tx.peek_char() == Some('@') {
                ...
                Ok(Term::Literal { v: serde_json::Value::String(s), dt: "rdf:langString".into(), lang: Some(lang) })
            } else if tx.peek_str_starts_with("^^") {
                tx.bump_char(); tx.bump_char();
                let dt = match tx.peek_char() {
                    ...
                    _ => {
                        ...
                        prefixes.get(&pfx).map(|v| format!("{v}{local}")).unwrap_or(format!("{pfx}:{local}"))
                    }
                };
                Ok(Term::Literal { v: serde_json::Value::String(s), dt, lang: None })
            } else {
                Ok(Term::Literal { v: serde_json::Value::String(s), dt: "xsd:string".into(), lang: None })
            }
        }
        Some(c) if c.is_ascii_alphabetic() || c == '_' => {
            ...
            let iri = prefixes.get(&pfx).map(|v| format!("{v}{local}"))
                .unwrap_or_else(|| format!("{pfx}:{local}"));
            Ok(Term::Iri(iri))
        }
        Some(c) if c.is_ascii_digit() || c == '-' => {
            let n: i64 = tx.read_number()? as i64;
            Ok(Term::Literal { v: serde_json::Value::Number(n.into()), dt: "xsd:integer".into(), lang: None })
        }
        other => Err(ParseError(format!("unexpected `{other:?}` while reading term"))),
    }
}

fn read_op(tx: &mut Tokenizer<'_>) -> Result<String, ParseError> {
    tx.skip_ws();
    let c = tx.bump_char().ok_or_else(|| ParseError("operator expected".into()))?;
    Ok(match c {
        '=' => "=".into(),
        '!' => { tx.expect('=')?; "!=".into() }
        '<' => if tx.peek_char() == Some('=') { tx.bump_char(); "<=".into() } else { "<".into() }
        '>' => if tx.peek_char() == Some('=') { tx.bump_char(); ">=".into() } else { ">".into() }
...
impl<'a> Tokenizer<'a> {
    ...
    fn peek_char(&self) -> Option<char> {
        if self.pos >= self.src.len() { None } else { Some(self.src[self.pos] as char) }
    }
    fn bump_char(&mut self) -> Option<char> {
        let c = self.peek_char()?; self.pos += 1; Some(c)
    }
    fn peek_str_starts_with(&self, s: &str) -> bool {
        self.src.get(self.pos..self.pos + s.len()).map_or(false, |b| b == s.as_bytes())
    }
    fn expect(&mut self, c: char) -> Result<(), ParseError> {
        self.skip_ws();
        if self.peek_char() == Some(c) { self.pos += 1; Ok(()) }
        else { Err(ParseError(format!("expected `{c}` at byte {}", self.pos))) }
    }
    fn read_ident(&mut self) -> Result<String, ParseError> {
        ...
        while self.pos < self.src.len() {
            let c = self.src[self.pos] as char;
            if c.is_ascii_alphanumeric() || c == '_' { self.pos += 1; } else { break; }
        }
        if start == self.pos { return Err(ParseError("ident expected".into())); }
    ...
    fn read_iri(&mut self) -> Result<String, ParseError> {
        self.skip_ws();
        if self.peek_char() != Some('<') { return Err(ParseError("`<` expected".into())); }
        self.pos += 1;
        let start = self.pos;
        while self.pos < self.src.len() && self.src[self.pos] as char != '>' { self.pos += 1; }
        if self.pos >= self.src.len() { return Err(ParseError("unterminated <iri>".into())); }
    ...
    fn read_number(&mut self) -> Result<u64, ParseError> {
        self.skip_ws();
        let start = self.pos;
        while self.pos < self.src.len() && (self.src[self.pos] as char).is_ascii_digit() { self.pos += 1; }
        if start == self.pos { return Err(ParseError("number expected".into())); }
        std::str::from_utf8(&self.src[start..self.pos]).unwrap().parse()
            .map_err(|_| ParseError("bad number".into()))
    }
    fn peek_word(&self) -> Option<String> {
        let mut p = self.pos;
        while p < self.src.len() && (self.src[p] as char).is_whitespace() { p += 1; }
        let start = p;
        while p < self.src.len() && (self.src[p] as char).is_ascii_alphabetic() { p += 1; }
        if start == p { return None; }
        Some(std::str::from_utf8(&self.src[start..p]).unwrap().to_string())
    }
    ...
    fn read_until(&mut self, end: char) -> Result<&str, ParseError> {
        self.skip_ws();
        let start = self.pos;
        while self.pos < self.src.len() && self.src[self.pos] as char != end { self.pos += 1; }
        if self.pos >= self.src.len() { return Err(ParseError(format!("`{end}` not found"))); }
        Ok(std::str::from_utf8(&self.src[start..self.pos]).unwrap())
    }
    // Suppress dead_code for the local() method's `c` shadowing - intentional.
    #[allow(dead_code)]
    fn _suppress(&self) -> &[u8] { self.src }
}
...
mod tests {
    ...
    fn parses_basic_select() {
        ...
        assert_eq!(q.patterns[0].predicate, Term::Iri("http://example.org/knows".into()));
    ...
    fn parses_graph_block() {
        ...
        "#).unwrap();
        assert_eq!(q.patterns.len(), 1);
        assert_eq!(q.patterns[0].graph, Some(Term::Iri("http://example.org/src".into())));
    }
}
