...
pub async fn register(
    ...
    match s.client.ensure_agent(
        &req.iri, &req.agent_type,
        req.label.as_deref(), req.model_id.as_deref(),
    ).await {
...
fn default_owner() -> String { "owner".to_string() }

pub async fn bind_context(
    State(s): State<Arc<AppState>>,
    Json(req): Json<BindContextReq>,
) -> impl IntoResponse {
    match s.client.bind_agent_context(req.agent_id, &req.context, &req.role).await {
