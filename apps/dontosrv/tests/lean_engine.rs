...
fn engine_path() -> Option<PathBuf> {
    ...
    p.pop();
    p.pop(); // apps/dontosrv/ → repo root
    p.push("packages/lean/.lake/build/bin/donto_engine");
...
async fn boot() -> Option<(Arc<dontosrv::AppState>, String)> {
    ...
    let lean = match dontosrv::lean::LeanClient::try_spawn(bin.to_str()).await {
        Ok(Some(c)) => c,
        Ok(None) => {
            eprintln!("lean: spawn returned None");
            return None;
        }
        Err(e) => {
            eprintln!("lean: spawn failed: {e}");
            return None;
        }
    };
...
async fn lean_parent_child_age_gap_finds_violation() {
    ...
    c.assert(
        &StatementInput::new("ex:alice", "ex:parentOf", Object::iri("ex:bob")).with_context(&ctx),
    )
    .await
    .unwrap();
    c.assert(
        &StatementInput::new(
            "ex:alice",
            "ex:birthYear",
            Object::lit(donto_client::Literal::integer(1850)),
        )
        .with_context(&ctx),
    )
    .await
    ...
    c.assert(
        &StatementInput::new(
            "ex:bob",
            "ex:birthYear",
            Object::lit(donto_client::Literal::integer(1855)),
        )
        .with_context(&ctx),
    )
    .await
...
async fn lean_parent_child_age_gap_passes_reasonable_pair() {
    ...
    c.assert(&StatementInput::new("ex:p1", "ex:parentOf", Object::iri("ex:c1")).with_context(&ctx))
        .await
        .unwrap();
    c.assert(
        &StatementInput::new(
            "ex:p1",
            "ex:birthYear",
            Object::lit(donto_client::Literal::integer(1900)),
        )
        .with_context(&ctx),
    )
    .await
    ...
    assert_eq!(
        viols.len(),
        0,
        "reasonable 25y gap should produce no violations: {v}"
    );
}
...
async fn lean_engine_is_required_for_lean_iris() {
    ...
    let _ = match client.pool().get().await {
        Ok(c) => c,
        Err(_) => {
            eprintln!("skip");
            return;
