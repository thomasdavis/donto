...
FILE="packages/lean/Donto/Theorems.lean"
BACKUP="${FILE}.bak"

step() { printf "\n\033[1;36m── %s ──\033[0m\n" "$*"; }

step "0. Baseline: lake build is green"
(cd packages/lean && PATH=$HOME/.elan/bin:$PATH lake build) 2>&1 | tail -3
...
set +e
(cd packages/lean && PATH=$HOME/.elan/bin:$PATH lake build) > /tmp/lake-fail.log 2>&1
RC=$?
set -e
echo "Exit code: $RC"
