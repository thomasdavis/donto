...
async fn setup_span(client: &donto_client::DontoClient, name: &str) -> (uuid::Uuid, uuid::Uuid) {
    ...
    let rev_id = client
        .add_revision(
            doc_id,
            Some("The model outperforms it on all benchmarks."),
            None,
            None,
        )
        .await
...
async fn create_mention_and_query() {
    ...
    let span_id = client
        .create_char_span(rev_id, 4, 9, Some("model"))
        .await
        .unwrap();

    let mention_id: uuid::Uuid = c
        .query_one(
    ...
    let rows = c
        .query(
            "select * from donto_mentions_in_revision($1, 'entity')",
            &[&rev_id],
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        rows[0].get::<_, Option<String>>("entity_iri").as_deref(),
        Some("model:mistral-7b")
    );
}
...
async fn mention_with_candidates() {
    ...
    let stored: Vec<String> = c
        .query_one(
            "select candidate_iris from donto_mention where mention_id = $1",
            &[&mention_id],
        )
        .await
        .unwrap()
...
async fn coref_cluster() {
    ...
    let span1 = client
        ...
        .unwrap();
    let span2 = client
        .create_char_span(rev_id, 22, 24, Some("it"))
        .await
        .unwrap();

    let m1: uuid::Uuid = c
        .query_one("select donto_create_mention($1, 'entity')", &[&span1])
        .await
        .unwrap()
        .get(0);
    let m2: uuid::Uuid = c
        .query_one("select donto_create_mention($1, 'entity')", &[&span2])
        .await
        .unwrap()
        .get(0);

    let cluster_id: uuid::Uuid = c
        .query_one(
    ...
    let members: i64 = c
        .query_one(
            "select count(*) from donto_coref_member where cluster_id = $1",
            &[&cluster_id],
        )
        .await
        .unwrap()
    ...
    let resolved: Option<String> = c
        .query_one(
            "select resolved_iri from donto_coref_cluster where cluster_id = $1",
            &[&cluster_id],
        )
        .await
        .unwrap()
...
async fn mention_type_validated() {
    ...
    let err = c
        ...
        .await
        .err()
        .expect("invalid mention_type must error");
...
async fn mentions_filter_by_type() {
    ...
    c.execute("select donto_create_mention($1, 'entity')", &[&s1])
        .await
        .unwrap();
    c.execute("select donto_create_mention($1, 'temporal')", &[&s2])
        .await
        .unwrap();

    let all: i64 = c
        .query_one(
            "select count(*) from donto_mentions_in_revision($1)",
            &[&rev_id],
        )
        .await
        .unwrap()
    ...
    let entities: i64 = c
        .query_one(
            "select count(*) from donto_mentions_in_revision($1, 'entity')",
            &[&rev_id],
        )
        .await
        .unwrap()
