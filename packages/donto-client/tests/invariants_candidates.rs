...
async fn promote_candidate() {
    ...
    client.ensure_context(&cand_ctx, "candidate", "permissive", None).await.unwrap();
    client.ensure_context(&target_ctx, "source", "permissive", None).await.unwrap();
    ...
    let promoted_id: uuid::Uuid = c.query_one(
    ...
    let original_open: bool = c.query_one(
        "select upper(tx_time) is null from donto_statement where statement_id = $1",
        &[&stmt_id],
    ).await.unwrap().get(0);
    assert!(!original_open, "candidate must be retracted after promotion");
    ...
    assert!(has_lineage, "promoted statement must track lineage to candidate");
}
...
async fn promote_non_candidate_fails() {
    ...
    client.ensure_context(&source_ctx, "source", "permissive", None).await.unwrap();
    ...
    let err = c.execute(
        "select donto_promote_candidate($1, 'anywhere')", &[&stmt_id],
    ).await.err().expect("promoting from non-candidate context must error");
...
async fn bulk_promote_above_threshold() {
    ...
    let promoted: i64 = c.query_one(
        "select donto_promote_candidates_above($1, $2, $3::double precision)",
        &[&cand_ctx, &target_ctx, &0.5f64],
    ).await.unwrap().get(0);
    assert_eq!(promoted, 2, "should promote 2 candidates (confidence >= 0.5)");

    // Verify remaining candidate
    let remaining: i64 = c.query_one(
...
async fn promoted_preserves_content() {
    ...
    let promoted_id: uuid::Uuid = c.query_one(
        "select donto_promote_candidate($1, $2)", &[&stmt_id, &target_ctx],
    ).await.unwrap().get(0);
    ...
    assert_eq!(row.get::<_, Option<String>>("object_iri").as_deref(), Some("ex:thing"));
