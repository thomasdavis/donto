...
async fn unit_conversion_cross_dimension_is_null() {
    ...
    let result: Option<f64> = c.query_one(
        "select donto_convert_unit(1, 'unit:second', 'unit:meter')", &[],
    ).await.unwrap().get(0);
    assert!(result.is_none(), "cross-dimension conversion must return null");
}
...
async fn normalize_percent() {
    ...
    // "60.1%" → 0.601
    let r: f64 = c.query_one("select donto_normalize_percent('60.1%')", &[]).await.unwrap().get(0);
    assert!((r - 0.601).abs() < 1e-6);

    // "0.601" → 0.601
    let r: f64 = c.query_one("select donto_normalize_percent('0.601')", &[]).await.unwrap().get(0);
    assert!((r - 0.601).abs() < 1e-6);

    // "100%" → 1.0
    let r: f64 = c.query_one("select donto_normalize_percent('100%')", &[]).await.unwrap().get(0);
    assert!((r - 1.0).abs() < 1e-6);

    // "0%" → 0.0
    let r: f64 = c.query_one("select donto_normalize_percent('0%')", &[]).await.unwrap().get(0);
    assert!(r.abs() < 1e-6);
}
...
async fn seeded_units_exist() {
    ...
    let count: i64 = c.query_one("select count(*) from donto_unit", &[]).await.unwrap().get(0);
    assert!(count >= 26, "expected at least 26 seeded units, got {count}");

    // Spot check a few
    for iri in ["unit:second", "unit:attosecond", "unit:percent", "unit:usd", "unit:kilogram"] {
        let exists: bool = c.query_one(
            "select exists(select 1 from donto_unit where iri = $1)", &[&iri],
        ).await.unwrap().get(0);
