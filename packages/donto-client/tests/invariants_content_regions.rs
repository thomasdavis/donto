...
async fn content_region_lifecycle() {
    ...
    let doc_id = client
        ...
        .unwrap();
    let rev_id = client
        .add_revision(doc_id, Some("text with figure"), None, None)
        .await
        .unwrap();

    let region_id: uuid::Uuid = c
        .query_one(
    ...
    assert_eq!(
        row.get::<_, Option<String>>("label").as_deref(),
        Some("Figure 1")
    );
    assert_eq!(
        row.get::<_, Option<String>>("caption").as_deref(),
        Some("Performance comparison chart")
    );
    assert_eq!(
        row.get::<_, Option<String>>("alt_text").as_deref(),
        Some("Bar chart showing MMLU scores")
    );
}
...
async fn content_region_types_validated() {
    ...
    let err = c
        ...
        .await
        .err()
        .expect("invalid region_type must error");
...
async fn content_region_with_section() {
    ...
    let doc_id = client
        ...
        .unwrap();
    let rev_id = client
        .add_revision(doc_id, Some("Results section with figure"), None, None)
        .await
        .unwrap();

    let sec_id: uuid::Uuid = c
        .query_one("select donto_add_section($1, 'Results')", &[&rev_id])
        .await
        .unwrap()
        .get(0);
    ...
    let stored_sec: Option<uuid::Uuid> = c
        .query_one(
