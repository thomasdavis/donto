...
async fn three_way_disagreement_about_birth_year_coexists() {
    ...
    for s in [&s_a, &s_b, &s_c] {
        c.ensure_context(s, "source", "permissive", None)
            .await
            .unwrap();
    }
    ...
    for (ctx, year) in claims {
        c.assert(
            &StatementInput::new(
                "ex:alice",
                "ex:birthYear",
                Object::lit(Literal::integer(year)),
            )
            .with_context(ctx),
        )
        .await
    ...
    assert_eq!(
        rows.len(),
        3,
        "three contradictory statements must all surface"
    );
...
async fn assert_and_negate_in_same_context_both_persist() {
    ...
    c.assert(
        &StatementInput::new("ex:alice", "ex:member", Object::iri("ex:club")).with_context(&ctx),
    )
    .await
    .unwrap();
    c.assert(
...
async fn polarity_table_invariants_exhaustive() {
    ...
    let n_any = c
        .match_pattern(None, Some("ex:p"), None, Some(&scope), None, 0, None, None)
        .await
        .unwrap()
        .len();
    assert_eq!(n_any, 4);
...
async fn idempotent_re_assert_does_not_double_count() {
    ...
    for _ in 0..50 {
        ids.insert(c.assert(&s).await.unwrap());
    }
    assert_eq!(
        ids.len(),
        1,
        "50 identical asserts must collapse to one statement_id"
    );
