...
async fn snapshot_member_count_matches_membership() {
    ...
    let conn = c.pool().get().await.unwrap();
    conn.execute(
        "select donto_snapshot_create($1, $2::jsonb, null)",
        &[&snap_iri.as_str(), &json!({"include":[ctx.clone()]})],
    )
    .await
    .unwrap();
...
async fn hypothesis_context_isolates_counterfactuals() {
    ...
    c.ensure_context(&curated_root, "custom", "permissive", None)
        .await
        .unwrap();
    c.ensure_context(&hypo_ctx, "hypothesis", "permissive", Some(&curated_root))
        .await
        .unwrap();

    // Curated fact: alice and alice_2 are different IRIs.
    c.assert(
        &StatementInput::new("ex:alice", "ex:type", Object::iri("ex:Person"))
            .with_context(&curated_root),
    )
    .await
    .unwrap();
    c.assert(
        &StatementInput::new("ex:alice_2", "ex:type", Object::iri("ex:Person"))
            .with_context(&curated_root),
    )
    .await
    .unwrap();
    ...
    c.assert(
        &StatementInput::new("ex:alice", "donto:sameAs", Object::iri("ex:alice_2"))
    ...
    assert_eq!(
        n_curated, 0,
        "curated view must not see hypothesis assertions"
    );
    ...
    assert_eq!(
        any_pred.len(),
        1,
        "hypothesis scope must see the sameAs assertion"
    );
}
...
async fn sameas_is_non_monotonic() {
    ...
    c.ensure_context(&ctx_with, "custom", "permissive", None)
        .await
        .unwrap();
    c.ensure_context(&ctx_without, "custom", "permissive", None)
        .await
        .unwrap();

    c.assert(
        &StatementInput::new("ex:alice", "donto:sameAs", Object::iri("ex:alice_2"))
            .with_context(&ctx_with),
    )
    .await
    .unwrap();
    ...
    let n = c
        .match_pattern(
            ...
            Some("donto:sameAs"),
            None,
            Some(&ContextScope::just(&ctx_with)),
            Some(Polarity::Asserted),
            0,
            None,
            None,
    ...
    let n = c
        .match_pattern(
            ...
            Some("donto:sameAs"),
            None,
            Some(&ContextScope::just(&ctx_without)),
            Some(Polarity::Asserted),
            0,
            None,
            None,
