...
async fn setup_span(client: &donto_client::DontoClient, name: &str) -> Uuid {
    ...
    let rev_id = client
        ...
        .await
        .unwrap();
    client.create_char_span(rev_id, 0, 3, Some("The")).await.unwrap()
}
...
async fn annotate_span_stores_feature_value() {
    ...
    let space_id: Uuid = c
        .query_one(
            "select donto_ensure_annotation_space($1)",
            &[&space_iri],
        )
        .await
        .unwrap()
    ...
    assert_eq!(row.get::<_, Option<String>>("value").as_deref(), Some("DET"));
...
async fn annotation_edge_links_two_annotations() {
    ...
    let rev_id = client.add_revision(doc_id, Some("The cat sat"), None, None).await.unwrap();

    let span_the = client.create_char_span(rev_id, 0, 3, Some("The")).await.unwrap();
    let span_cat = client.create_char_span(rev_id, 4, 7, Some("cat")).await.unwrap();
