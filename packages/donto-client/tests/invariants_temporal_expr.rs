...
async fn temporal_expression_lifecycle() {
    ...
    let rev_id = client.add_revision(doc_id, Some("In 2023, the model was released."), None, None).await.unwrap();
    let span_id = client.create_char_span(rev_id, 3, 7, Some("2023")).await.unwrap();

    let expr_id: uuid::Uuid = c.query_one(
    ...
    assert_eq!(row.get::<_, NaiveDate>("resolved_from"), NaiveDate::from_ymd_opt(2023, 1, 1).unwrap());
    assert_eq!(row.get::<_, NaiveDate>("resolved_to"), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap());
...
async fn temporal_range_query() {
    ...
    let doc_id = client.ensure_document(&format!("test:doc/{prefix}"), "text/plain", None, None, None).await.unwrap();
    let rev_id = client.add_revision(doc_id, Some("1850 and 2023"), None, None).await.unwrap();

    let s1 = client.create_char_span(rev_id, 0, 4, Some("1850")).await.unwrap();
    let s2 = client.create_char_span(rev_id, 9, 13, Some("2023")).await.unwrap();
...
async fn approximate_temporal_expression() {
    ...
    let doc_id = client.ensure_document(&format!("test:doc/{prefix}"), "text/plain", None, None, None).await.unwrap();
    let rev_id = client.add_revision(doc_id, Some("circa 1850"), None, None).await.unwrap();
    let span_id = client.create_char_span(rev_id, 0, 10, Some("circa 1850")).await.unwrap();
