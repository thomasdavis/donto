...
fn parse_doc(doc: &Doc, default_context: &str, prefix: &str) -> Vec<StatementInput> {
    let mut out = Vec::new();
    for n in &doc.nodes {
    ...
    for e in &doc.edges {
        ...
        push(
            ...
            &id,
            &format!("{prefix}from"),
            Object::iri(format!("{prefix}{}", e.from)),
            default_context,
        );
        push(
            &mut out,
