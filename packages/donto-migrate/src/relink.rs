...
pub async fn relink(client: &DontoClient, sqlite: &Path, root: &str) -> Result<RelinkReport> {
    ...
    if has_table(&conn, "documents")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            if let Some(t) = title {
                buf.push(s(&iri, "rdfs:label", obj_str(&t), root, None));
            }
            if let Some(t) = doc_type {
                buf.push(s(&iri, "donto:docType", obj_str(&t), root, None));
            }
            ...
            if let Some(h) = content_hash {
                buf.push(s(&iri, "donto:contentHash", obj_str(&h), root, None));
            }
            if let Some(t) = tags {
                if !t.is_empty() && t != "[]" {
    ...
    if has_table(&conn, "chunks")? {
        eprintln!("chunks…");
        let mut q = conn.prepare("select id, document_id, chunk_index, content from chunks")?;
        let mut rows = q.query([])?;
        let mut n = 0u64;
        while let Some(r) = rows.next()? {
            let id: i64 = r.get(0)?;
            ...
            buf.push(s(
                &iri,
                "donto:document",
                obj_iri(format!("{root}/document/{doc}")),
                root,
                None,
            ));
            buf.push(s(
                &iri,
                "donto:chunkIndex",
                Object::lit(Literal::integer(idx)),
                root,
                None,
            ));
            ...
            if content.len() > excerpt.len() {
                buf.push(s(
                    &iri,
                    "donto:contentTruncated",
                    obj_int(content.chars().count() as i64),
                    root,
                    None,
                ));
            }
    ...
    if has_table(&conn, "claims")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            buf.push(s_v(
                &iri,
                "donto:claimSubject",
                obj_iri(format!("{root}/iri/{subject_id}")),
                root,
                valid,
            ));
            buf.push(s_v(
                &iri,
                "donto:claimPredicate",
                obj_str(&predicate),
                root,
                valid,
            ));
            ...
            if let Some(e) = object_entity_id.as_deref() {
                buf.push(s_v(
                    &iri,
                    "donto:claimObjectEntity",
                    obj_iri(format!("{root}/iri/{e}")),
                    root,
                    valid,
                ));
            }
            buf.push(s_v(
                &iri,
                "donto:sourceContext",
                obj_iri(format!("{root}/source/{source_id}")),
                root,
                valid,
            ));
    ...
    if has_table(&conn, "discrepancies")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            buf.push(s(
                &iri,
                "donto:onSubject",
                obj_iri(format!("{root}/iri/{subject}")),
                root,
                None,
            ));
            buf.push(s(
            ...
            if let Some(r) = resolution.as_deref() {
                buf.push(s(&iri, "donto:resolution", obj_str(r), root, None));
            }
            if let Some(r) = resolution_rule.as_deref() {
                buf.push(s(&iri, "donto:resolutionRule", obj_str(r), root, None));
            }
            if let Some(p) = preferred_claim.as_deref() {
                buf.push(s(
                    &iri,
                    "donto:preferredClaim",
                    obj_iri(format!("{root}/claim/{p}")),
                    root,
                    None,
                ));
            }
            if let Some(c) = created_at.as_deref() {
                buf.push(s(&iri, "donto:createdAt", obj_str(c), root, None));
            }
            n += 1;
            if n % 50_000 == 0 {
    ...
    if has_table(&conn, "hypotheses")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            buf.push(s(&iri, "donto:status", obj_str(&status), root, None));
            if let Some(v) = ev_for.as_deref() {
                buf.push(s(&iri, "donto:evidenceFor", obj_str(v), root, None));
            }
            if let Some(v) = ev_against.as_deref() {
                buf.push(s(&iri, "donto:evidenceAgainst", obj_str(v), root, None));
            }
            if let Some(v) = decisive.as_deref() {
                buf.push(s(&iri, "donto:decisiveRecord", obj_str(v), root, None));
            }
    ...
    if has_table(&conn, "relationships")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            buf.push(s_v(
                &iri,
                "donto:entityB",
                obj_iri(format!("{root}/iri/{b}")),
                root,
                valid,
            ));
            if let Some(s2) = src.as_deref() {
                buf.push(s_v(
                    &iri,
                    "donto:sourceContext",
                    obj_iri(format!("{root}/source/{s2}")),
                    root,
                    valid,
                ));
            }
            if let Some(c) = conf.as_deref() {
                buf.push(s_v(&iri, "donto:confidence", obj_str(c), root, valid));
            }
            if let Some(c) = created.as_deref() {
                buf.push(s_v(&iri, "donto:createdAt", obj_str(c), root, valid));
            }
            n += 1;
            if n % 50_000 == 0 {
    ...
    if has_table(&conn, "events")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            buf.push(s_v(
                &iri,
                "rdf:type",
                obj_iri(format!("ex:{typ}")),
                root,
                (d, None),
            ));
            if let Some(dv) = date.as_deref() {
                buf.push(s_v(&iri, "donto:dateValue", obj_str(dv), root, (d, None)));
            }
            ...
            if let Some(l) = loc.as_deref() {
                buf.push(s_v(
                    &iri,
                    "donto:location",
                    obj_iri(format!("{root}/iri/{l}")),
                    root,
                    (d, None),
                ));
            }
            if let Some(de) = desc.as_deref() {
                buf.push(s_v(&iri, "donto:description", obj_str(de), root, (d, None)));
            }
            if let Some(s2) = src.as_deref() {
                buf.push(s_v(
                    &iri,
            ...
            if let Some(c) = created.as_deref() {
                buf.push(s_v(&iri, "donto:createdAt", obj_str(c), root, (d, None)));
            }
            n += 1;
            if n % 50_000 == 0 {
                eprintln!("  …{n} events");
    ...
    if has_table(&conn, "aliases")? {
        ...
        let mut q = conn
            .prepare("select id, entity_id, name_form, year, location, source_id from aliases")?;
        let mut rows = q.query([])?;
        let mut n = 0u64;
        while let Some(r) = rows.next()? {
            let id: i64 = r.get(0)?;
            ...
            buf.push(s_v(
                &iri,
                "donto:entity",
                obj_iri(format!("{root}/iri/{entity}")),
                root,
                valid,
            ));
            buf.push(s_v(&iri, "donto:nameForm", obj_str(&name), root, valid));
            if let Some(l) = location.as_deref() {
                buf.push(s_v(&iri, "donto:location", obj_str(l), root, valid));
            }
            if let Some(s2) = src.as_deref() {
                buf.push(s_v(
                    &iri,
    ...
    if has_table(&conn, "ingestion_log")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            buf.push(s(
                &iri,
                "donto:relationshipsExtracted",
                obj_int(rel),
                root,
                None,
            ));
            if let Some(st) = status.as_deref() {
                buf.push(s(&iri, "donto:status", obj_str(st), root, None));
            }
            if let Some(n2) = notes.as_deref() {
                buf.push(s(&iri, "donto:notes", obj_str(n2), root, None));
            }
            n += 1;
            if buf.len() >= BATCH {
                flush!(report, client, buf);
    ...
    if has_table(&conn, "persons")? {
        ...
        while let Some(r) = rows.next()? {
            ...
            if let Some(c) = conf.as_deref() {
                buf.push(s(&iri, "donto:confidence", obj_str(c), root, None));
            }
            if let Some(y) = by {
                buf.push(s(&iri, "donto:birthYearEst", obj_int(y), root, None));
            }
            if let Some(y) = dy {
                buf.push(s(&iri, "donto:deathYearEst", obj_int(y), root, None));
            }
            if let Some(n2) = notes.as_deref() {
                buf.push(s(&iri, "donto:notes", obj_str(n2), root, None));
            }
...
fn s_v(
    ...
    o: Object,
    ctx: &str,
    valid: (Option<NaiveDate>, Option<NaiveDate>),
) -> StatementInput {
...
fn obj_str(s: &str) -> Object {
    Object::lit(Literal::string(scrub(s)))
}
...
fn obj_int(n: i64) -> Object {
    Object::lit(Literal::integer(n))
}
...
fn obj_iri(s: impl Into<String>) -> Object {
    Object::iri(s.into())
}
