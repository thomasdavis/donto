...
- One Postgres node handles the v1 target of 10⁹ statements per
  [PRD §25 H1](../PRD.md#25-performance-hypotheses). 128 GB RAM is the
  reference.
...
## Sidecar operational contract (PRD §15)
...
| `donto_assert` returns the same UUID twice | Idempotent re-assert (expected) | this is by design (PRD §19) |
...
Default indexes (PRD §14) cover the documented access patterns. To enable
...
For corpora at the H1 target (10⁹ statements), partition `donto_statement`
