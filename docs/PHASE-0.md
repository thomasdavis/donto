...
Per PRD §26 Phase 0:
- Pure Postgres prototype of `donto_statement` and context tables.
- Rust client for `donto_assert`, `donto_query`, `donto_retract`.
- N-Quads loader; sample LUBM subset.
- Bitemporal correctness tests.

**Exit criteria:** round-trip N-Quads, scoped pattern query, retraction, bitemporal time-travel.

## What's in scope

- `donto_statement` physical row exactly as PRD §5 (subject, predicate, object_iri/object_lit, context, tx_time, valid_time, flags packing polarity + maturity).
- `donto_context` table per PRD §7, with `kind`, `parent`, `mode`.
- `donto_iri` side table (PRD §5) — but Phase 0 stores IRIs as text directly; the 128-bit hash optimization is Phase 1+.
- One annotation overlay: `donto_stmt_lineage` (needed for retraction history reasoning). Other overlays are stubbed.
- A small set of plpgsql functions: `donto_assert`, `donto_assert_batch`, `donto_retract`, `donto_correct`, `donto_match` (subject/predicate/object pattern), `donto_resolve_scope`.
- Rust crate `donto-client` over `tokio-postgres` exposing the same surface as a typed API.
- N-Quads parser (`oxrdf` / `rio_turtle`).
